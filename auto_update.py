...
def add_update_controls(dialog) -> None:
    ...
    label.setStyleSheet("color:rgba(255,255,255,0.55); margin-top:8px;")
...
def _check_from_dialog(dialog, button: QPushButton) -> None:
    if not getattr(sys, "frozen", False):
        QMessageBox.information(dialog, "Обновления", "Обновление доступно только в собранной EXE-версии.")
        return
    ...
    def checked(release: dict):
        ...
        if _version_tuple(release["version"]) <= _version_tuple(__version__):
            QMessageBox.information(dialog, "Обновления", f"У вас актуальная версия {__version__}.")
            return
        answer = QMessageBox.question(
            dialog,
            "Доступно обновление",
...
def _show_error(dialog, button: QPushButton | None, error: str) -> None:
    if button is not None:
        button.setEnabled(True)
        button.setText("Проверить обновления")
    QMessageBox.critical(dialog, "Ошибка обновления", f"Не удалось обновить ActPilot:\n{error}")
